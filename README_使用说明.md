...
这是一个面向桌面六轴机械臂的离线多任务操作仿真工程。项目将视觉目标坐标、任务条件、Action-Chunk Transformer、扩散轨迹生成、POE/URDF 运动学核验、ROS 2 规划入口和硬件执行门禁放在同一条可审计链路中。默认只生成离线规划和验证报告，不会连接或驱动真实机械臂。
...
- **具身 Action-Chunk Transformer**：抓取位姿、放置目标、观测不确定性、任务 profile 和可学习任务 token 经两层 Transformer 编码；三层交叉注意力解码器一次输出 `32 x 6` 六轴关节动作块。
...
正式模型使用 `hidden_dim=128`，共有 1,320,710 个可训练参数。使用固定随机种子完成如下离线基准：
...
| 指标 | 结果 |
| --- | ---: |
| 原始策略抓取端点平均误差 | 6.62 mm |
| 约束投影后抓取端点平均误差 | 4.18 mm |
| 五类任务总体投影安全覆盖率 | 82.34% |
| 高噪声 OOD 输入拒绝率 | 100.00% |

逐任务投影安全覆盖率：转运 83.59%、分拣区 A 84.38%、分拣区 B 78.13%、检查扫描 82.03%、精密插入 83.59%。这些是由已纳入工程的 POE 模型、合成视觉噪声和 TCP 场景审查得到的离线结果，不代表真机抓取成功率。
