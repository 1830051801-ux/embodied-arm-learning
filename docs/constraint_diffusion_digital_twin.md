# Multi-Task Embodied Action-Chunk Transformer Benchmark

## Objective

This experiment evaluates whether a task-conditioned trajectory policy can generate useful offline six-axis action chunks across multiple process templates while preserving explicit kinematic and scene safety checks. It is a digital-twin benchmark, not authorization for physical movement.
...
The `multitask_embodied_action_chunk_transformer_diffusion` checkpoint uses 1,320,710 trainable parameters at `hidden_dim=128`.
...
3. Thirty-two learned six-axis action queries cross-attend to the context through a three-layer Transformer decoder and predict a trajectory proposal.
4. Noisy joint-state tokens with timestep embeddings use the same decoder to predict diffusion noise over the entire `32 x 6` action chunk.
5. Inference begins from the learned proposal and runs 16 denoising steps before the safety projection stage.
...
| Task | Raw endpoint error | Projected endpoint error | Projected-safe coverage | Shift OOD abstention |
| --- | ---: | ---: | ---: | ---: |
| Transfer | 6.96 mm | 4.19 mm | 83.59% | 100.00% |
| Sort zone A | 6.79 mm | 4.11 mm | 84.38% | 100.00% |
| Sort zone B | 6.56 mm | 4.40 mm | 78.13% | 100.00% |
| Inspection scan | 6.50 mm | 4.19 mm | 82.03% | 100.00% |
| Precision insert | 6.28 mm | 4.01 mm | 83.59% | 100.00% |
| **Overall** | **6.62 mm** | **4.18 mm** | **82.34%** | **100.00%** |
